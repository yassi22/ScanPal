...
**Tier 2 — echt nieuw, gemiddelde kost:** Supabase/Firebase exposed-config · subdomain-takeover · WAF/CDN+API-rate-limit-inspectie · threat-intelligence/reputatie-lookup. Threat intelligence (75) is ✅ opgeleverd; zie het gap-document voor de overige scope en volgorde. **Tier 3 — nieuwe actieve tests (opt-in+Pro):** auth-flow-scanner (eigendom-geverifieerd) · file-upload-scanner. **Parkeren:** audit-logging (black-box niet zinvol te definiëren).
