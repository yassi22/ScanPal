...
- Per-site of per-team-statuspagina (een team met 10 sites → 10 pagina's of 1 overzicht?) — eerst per-site (eenvoudig), team-overzicht later
...
- SSR-caching: de pagina is read-only en kan per request gepubliceerd worden zonder cache — of 10s `stale-while-revalidate`?

## Acceptatiecriteria

- [ ] Publieke pagina toont status, uptime-%, 30/90-dagen-grafiek en incidenten zonder login
- [ ] Toggle + deel-URL werken; slug is niet raadbaar en niet geïndexeerd
- [ ] Publieke data bevat nooit findings/scores/PII; JSON-feed werkt met rate limiting
- [ ] Alleen teamleden kunnen de toggle aanpassen; pagina bestaat alleen voor actieve slugs (404 anders)
